...
> API-Testing-Suite - Open source tool by AetherCodeHQ

`api` `cli` `developer-tools` `golang` `open-source` `testing`

## What is it?

**Api Testing Suite** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/API-Testing-Suite.git
...
```bash
# Basic usage
./API-Testing-Suite --path ./target

# With options
./API-Testing-Suite --path ./target --format json --output report.json

# Verbose mode
./API-Testing-Suite --path ./target --verbose
```
...
### Basic scan
```bash
./API-Testing-Suite --path ./my-project
```

### JSON report
```bash
./API-Testing-Suite --path ./my-project --format json --output report.json
```
...
- name: Run Api Testing Suite
  run: ./API-Testing-Suite --path . --format json --output report.json
```
